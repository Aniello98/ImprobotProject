...
class ActorInfo:
    ...
    def __init__(self):
        ...
        if self.actor_input not in ["stdin", "world"]:  # load actor behaviour from json file
            ...
            self.rst_sub = rospy.Subscriber("/reset_ai_server", Empty, self.reset)
    ...
    def update_actor_info(self, msg):
        ...
        self.actor_history.append({
            "actor_pos_x" : round(m["actorPosX"], 5),
            "actor_pos_y" : round(m["actorPosY"], 5),
            "scenic_action" : m["scenicAction"].lower(),
            "time" : rospy.get_rostime()
        })

        # consider just the last 30 elements
        if len(self.actor_history) > 30:
    ...
    def analyze_history(self):
        ...
        if self.scenic_action == "none":
            self.lock.release()
            return False #waiting until it doesn't detect a concrete action
    ...
    def reset(self, msg):
        rospy.loginfo(">>> RESETTING AI SERVER")
        self.counter = 0
        self.up_to_date = False
        self.actor_history = []
        self.actor_pos_x = self.actor_pos_y = self.actor_orientation = 0
